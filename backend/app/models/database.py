...
class InterviewAnalysis(Base):
    """
    Post-session behavioral analysis generated by the InterviewBehaviorAnalyzer.
    Stores dimension scores, behavioral patterns, red flags, and recommendations.
    """
    ...
    technical_mastery_score = Column(Float, nullable=True)
    emotional_stability_score = Column(Float, nullable=True)
    authenticity_score = Column(Float, nullable=True)
    self_confidence_score = Column(Float, nullable=True)
    communication_score = Column(Float, nullable=True)
    overall_score = Column(Float, nullable=True)

    # Behavioral patterns detected (JSON list)
    behavioral_patterns = Column(JSON, nullable=True)
    # [{"type": "rapid_confusion", "timestamp": 45.2, "severity": "high", "description": "...", "context_question": "..."}]

    # Red flags (JSON list)  
    red_flags = Column(JSON, nullable=True)
    # [{"type": "social_masking", "evidence": "...", "severity": "medium"}]

    # Question-emotion correlations (JSON list)
    question_correlations = Column(JSON, nullable=True)
    # [{"question_timestamp": 30.0, "question_text": "...", "pre_emotion": "confident", "post_emotion": "nervous", "emotion_shift": "negative", "insight": "..."}]

    # AI-generated recommendations (JSON list)
