...
class MicroExpressionEngine:
    ...
    def __init__(self):
        # Rolling buffer of recent AU readings (last ~2 seconds)
        ...
        self._min_detection_gap = 0.5  # Min seconds between detections
    ...
    def analyze(
        ...
        reading = AUReading(timestamp=timestamp, values=current_aus.copy())
        ...
        event = self._detect_micro_expression(timestamp, dominant_emotion)
        return event
    ...
    def _detect_micro_expression(
        ...
        for au_name in aus_to_track:
            result = self._check_au_pattern(au_name, timestamp)
            if result is not None:
                activated_aus.append(au_name)
                activation_durations.append(result)
        ...
        # Check minimum gap between detections
    ...
    def _check_au_pattern(self, au_name: str, current_time: float) -> float | None:
        """
        Check if an AU shows an onset-apex-offset pattern in the buffer.
        
        Returns the duration in seconds if pattern detected, else None.
        """
        if len(self.au_buffer) < 5:
            return None
        ...
        threshold = 0.3  # Minimum AU activation to consider
